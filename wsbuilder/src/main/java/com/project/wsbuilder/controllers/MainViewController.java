...
public class MainViewController {
    ...
    private static final HBox[] COLUMN_TITLES = {
            createColumnTitle(" Trending Extension", "/com/project/wsbuilder/files/images/trending-icon.png"),
            createColumnTitle(" Recom. Extension", "/com/project/wsbuilder/files/images/download-icon.png"),
    ...
    private void loadTrendingExtensions(VBox column) {
        ...
        if (response != null) {
            ...
            if (results.length() > 0) {
                ...
                for (int i = 0; i < Math.min(9, trendingExtensions.size()); i++) {
                    JSONObject extension = trendingExtensions.get(i);
                    HBox item = new HBox();
                    item.setPrefWidth(column.getPrefWidth() - 10);
                    item.getStyleClass().add("item-content");
                    if (i < 3) {
                        ImageView icon = new ImageView(new Image(getClass().getResource(medalImages[i]).toExternalForm()));
    ...
    private void loadRecommendedExtensions(VBox column) {
        ...
        if (response != null) {
            ...
            if (results.length() > 0) {
                ...
                for (int i = 0; i < Math.min(9, recommendedExtensions.size()); i++) {
